...
type PortForwardASvcRequest struct {
	// RestConfig is the kubernetes config
	RestConfig *rest.Config
	// Service is the selected svc for this port forwarding
	Service v1.Service
	// LocalPort is the local port that will be selected to expose the ServicePort
	LocalPort int
	// ServicePort is the target port for the svc
	ServicePort int
	// Steams configures where to write or read input from
	Streams genericclioptions.IOStreams
	// StopCh is the channel used to manage the port forward lifecycle
...
func main() {
	...
	//go func() {
	//	// PortForward the svc specified from its port 9090 to the local port
	//	// 8085
	//	err := PortForwardAService(PortForwardASvcRequest{
	//		RestConfig: config,
	//		Service: v1.Service{
...
func PortForwardAService(req PortForwardASvcRequest) error {
